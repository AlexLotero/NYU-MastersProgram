...
void print_gift_card_info(struct this_gift_card *thisone) {
	...
	//fuzzer2
	int total_value = get_gift_card_value(thisone);
...
struct this_gift_card *gift_card_reader(FILE *input_fd) {
	...
	while (!feof(input_fd)) {
		...
		if (ret_val->num_bytes < 0) {
                        printf("Invalid Gift Card Provided.\n"); 
			printf("Number of bytes must be greater than or equal to 0.\n %d is not a valid number of bytes.\n", ret_val->num_bytes);
			exit(0);
                }
...
int main(int argc, char **argv) {
	...
	//crash1
	if (argc < 3 || !(input_fd)) {
		printf("Incorrect argument format or invalid arguments provided.\n\n");
		printf("The gift card file must already exist within the directory.\n\n");
		printf("The correct format is as follows: \n");
		printf("compiledCode number1or2 giftCardFile\n\n");
		printf("Example: \n");
		printf("./giftcardreader 1 examplefile.gft\n\n");
		return 0;
	} 
